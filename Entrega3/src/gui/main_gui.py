...
class PoseTrackGUI:
    ...
    def update_configs(self):
        """Actualiza las configuraciones en tiempo real después de guardar"""
        try:
            # Reinicializar componentes que dependen de las configuraciones
            if hasattr(self, 'pose_tracker'):
                # Actualizar configuración de MediaPipe
                self.pose_tracker.pose = self.pose_tracker.mp_pose.Pose(
            ...
            # Si la cámara está activa, aplicar nuevas configuraciones
            if self.camera_active and self.cap:
                self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, CAMERA_CONFIG["width"])
                self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, CAMERA_CONFIG["height"])
                self.cap.set(cv2.CAP_PROP_FPS, CAMERA_CONFIG["fps"])
            
            self.log_message("Configuraciones actualizadas exitosamente")
            
        except Exception as e:
            self.log_message(f"Error actualizando configuraciones: {e}")
...
class SettingsDialog:
    ...
    def setup_classification_settings(self, parent):
        ...
        ).pack(side=tk.RIGHT)        # Buffer de suavizado
    ...
    def save_settings(self):
        ...
        try:
            ...
            # Guardar configuración de MediaPipe
            config_manager.save_mediapipe_config(
                min_detection_confidence=self.min_confidence_var.get(),
                model_complexity=self.complexity_var.get()
            )
            
            # Guardar configuración de características
            config_manager.save_feature_config(
                window_size=self.window_size_var.get(),
                buffer_size=self.buffer_size_var.get()
            )
            
            # Notificar a la aplicación principal que las configuraciones han cambiado
            if hasattr(self.main_app, 'update_configs'):
                self.main_app.update_configs()
